<template>
  <ClientOnly>
    <div class="fullscreen-video-scroller fixed inset-0 overflow-hidden bg-transparent">
      <div
        v-if="isLoading"
        class="absolute inset-0 z-20 flex flex-col items-center justify-center bg-black/80 text-white p-4 text-center"
      >
        <p class="text-xl font-semibold mb-2">{{ t('loading.videos') }}</p>
      </div>
      <div
        v-else-if="error"
        class="absolute inset-0 z-20 flex flex-col items-center justify-center bg-red-900/80 text-white p-4 text-center"
      >
        <p class="text-xl font-semibold mb-2">{{ t('error.loading_videos') }}</p>
        <p>{{ error }}</p>
      </div>
      <div
        v-else-if="ui.showVideo && allVideoUrls.length === 0"
        class="absolute inset-0 z-20 flex flex-col items-center justify-center bg-red-900/80 text-white p-4 text-center"
      >
        <p class="text-xl font-semibold mb-2">{{ t('error.no_videos') }}</p>
        <p>{{ t('error.no_videos_in_bucket') }}</p>
      </div>
      <div
          ...
          >
            ...
            >
              <video
                ...
                @error="(e) => console.error('Video loading error:', videoItem.src, e)"
              >
      ...
      <div v-if="!ui.showVideo" class="absolute inset-0 z-[-1] flex items-center justify-center bg-black/10">
        <p class="text-white/70">{{ t('info.no_videos_available') }}</p>
      </div>
...
const fetchVideos = async () => {
  try {
    ...
    error.value = null;
    const response = await $fetch<string[]>('/api/assets/video-grid');
    allVideoUrls.value = response;
  } catch (err) {
...
const calculateLayoutAndVideos = async () => {
  ...
  if (props.numColumns <= 0 || allVideoUrls.value.length === 0) {
    columnsData.value = [];
    return;
  }
  ...
  const videosToDisplayPerColumnSet = Math.max(2, Math.ceil(videosNeededPerScreenHeight * props.screensPerColumn * 2));
  ...
  for (let i = 0; i < props.numColumns; i++) {
    ...
    for (let j = 0; j < videosToDisplayPerColumnSet; j++) {
      const videoIndex = globalVideoIndex % allVideoUrls.value.length;
      const src = allVideoUrls.value[videoIndex];
